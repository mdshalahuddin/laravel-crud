...
<body>
    <div class="container">
        <div class="flex justify-between my-5">
            <h2 class="text-xl text-red-500">Edit</h2>
            <a href="/" class="px-4 py-2 text-white bg-green-600 rounded">Back Post</a>
        </div>
        <div class="">
            <form action="{{ route('update', $ourPost->id) }}" method="POST" class="space-y-6"
                enctype="multipart/form-data">
                @csrf
                <!-- Name Field -->
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-700">Name</label>
                    <input type="text" value="{{ $ourPost->name }}" id="name" name="name"
                ...
                <div>
                    ...
                    <input type="type" id="description" value="{{ $ourPost->description }}" name="description"
                        class="block w-full px-4 py-2 mt-1 border border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
